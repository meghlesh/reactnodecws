...
function AdminWeeklyOffSetup() {
  ...
  useEffect(() => {
    const fetchWeeklyOff = async () => {
      try {
        const res = await axios.get(` https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/admin/weeklyoff/${year}`);
  ...
  const saveWeeklyOff = async () => {
    try {
      await axios.post(" https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/admin/weeklyoff", { year, saturdays });
...
>Set Saturday Off </h5> 
          ...
          <button
            ...
            style={{ borderRadius: "6px", backgroundColor: "#3A5FBF" }}
          >
            Save
          </button>
        </div>
