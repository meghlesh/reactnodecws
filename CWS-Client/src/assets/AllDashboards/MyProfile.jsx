...
function MyProfile({ user }) {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const res = await axios.get(` https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/employees/${user._id}`);
        setProfile(res.data);
        setFormData({
          ...res.data,
...
// 🧩 Contact validation — must be exactly 10 digits
  ...
  if ((currentZip && !/^\d{6}$/.test(currentZip)) || (permanentZip && !/^\d{6}$/.test(permanentZip))) {
    ...
    try {
      ...
      await axios.put(` https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/employees/${user._id}`, data, {
      ...
      // Refresh profile
      const updated = await axios.get(` https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/employees/${user._id}`);
  ...
  const renderFileLink = (file) => {
    ...
    return (
      <a href={` https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/uploads/${file}`} target="_blank" rel="noreferrer">
        View PDF
      </a>
    );
  };
              ...
              {isEditing ? (
                <div className="d-flex flex-column align-items-center text-center">
                  ...
                  {formData.image instanceof File ? (
                    <p className="text-muted small mb-0">Selected: {formData.image.name}</p>
                  ) : profile.image ? (
                    <img
                      // src={` https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/${profile.image}`}
                      src={profile.image?.startsWith("http")
                        ? profile.image
                        : `https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/image/${profile.image}`}
                      alt="Profile"
                      style={{
                        width: "100px",
                        height: "100px",
      ...
      {formData.image instanceof File ? (
        ...
        <p className="text-muted small mb-0 mt-1">No image selected</p>
          ...
          <div className="d-flex gap-2">
            {!isEditing ? (
              <button className="btn  btn-sm px-3 px-md-4" style={{ background: '#3A5FBE', borderColor: '#3A5FBE', color: "white" }} onClick={() => setIsEditing(true)}>
                Edit
              </button>
            ) : (
              <>
                <button className="btn  btn-sm px-3 px-md-4" style={{ background: '#3A5FBE', borderColor: '#3A5FBE', color: "white" }} onClick={handleSave}
                >                  Save
                </button>
                <button
                  className="btn  btn-sm px-3 px-md-4" style={{ background: '#3A5FBE', borderColor: '#3A5FBE', color: "white" }}
                  onClick={() => {
                    setFormData(profile);
                    setIsEditing(false);
                  }}
    ...
    textTransform: field === "email" ? "none" : "capitalize", // ✅ don't capitalize email
          ...
          <div className="d-flex gap-3 flex-wrap">
            {["aadharCardPdf", "panCardPdf", "appointmentLetter"].map((field) => (
              <div key={field} className="d-flex flex-column align-items-center">
                {/* <strong className="mb-1">{field}</strong> */}
                <label className="form-label text-primary">{formatLabel(field)}</label>
                {isEditing ? (
                  <input type="file" name={field} onChange={handleChange} />
                ) : profile[field] ? (
                  // <a
                  //   // href={` https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/uploads/${profile[field]}`}

                  //   href={
                  //     profile[field]?.startsWith("http")
                  //       ? profile[field]
                  //       : `https://api-nodelinux-be-b8cpf8cbe8f4f5b0.centralus-01.azurewebsites.net/uploads/${profile[field]}`
                  //   } target="_blank"
                  //   rel="noopener noreferrer"
                  // >
                  //   <img src="/pdfimg.png" alt={field} style={{ width: "50px" }} />
                  // </a>
                  ...
                  <a
                    href={
                      profile[field]?.startsWith("http")
                        ? profile[field]
                        : `https://res.cloudinary.com/dfvumzr0q/raw/upload/${profile[field]}`
                    }
                    target="_blank"
                    rel="noopener noreferrer"
                  >
            ...
            <div className="d-flex flex-column align-items-center">
              ...
              {isEditing ? (
                <input type="file" name="passbookPdf" onChange={handleChange} />
              ) : profile.bankDetails?.passbookPdf ? (
                <a
                  href={
                    `https://res.cloudinary.com/dfvumzr0q/raw/upload/${profile.bankDetails.passbookPdf}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <img src="/pdfimg.png" style={{ width: "50px" }} />
                </a>
              ) : (
                "-"
              )}
